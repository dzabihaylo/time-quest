...
<task type="auto">
  ...
  <action>
    **Update `TimeQuest/Features/Player/ViewModels/GameSessionViewModel.swift`:**
    ...
    6. Note: `completeActiveTask()` tracks the max difficulty level via `sessionMaxDifficultyLevel` but does NOT stamp the session — that happens in `advanceToNextTask()` (step 5 above). The session is only stamped once, after all tasks complete.
    ...
    **Update `generate-xcodeproj.js`:**
    ...
    ```bash
    cd /Users/davezabihaylo/Documents/Projects/time-quest && node generate-xcodeproj.js
    ```

    Then verify the build compiles:
    ```bash
    cd /Users/davezabihaylo/Documents/Projects/time-quest/TimeQuest && xcodebuild -scheme TimeQuest -destination 'platform=iOS Simulator,name=iPhone 16' build 2>&1 | tail -20
    ```
